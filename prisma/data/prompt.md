...
以下のフォーマットに従い、JavaScript の学習カリキュラムの lesson データを作成してください。
初級、中級、上級の３つを作ってください。

【条件】

- JavaScript の学習レベル: **初級 / 中級 / 上級**
- レッスン名: 各レベルに適したレッスン名を指定
- 各レッスンに 3 つ以上の問題を含める
- 注意点 (`caution`): コーディングのルールを明確に記述
  - 関数の定義 → 定数の定義 → 実行結果の出力の順に書く
  - アロー関数を用いる
  - 配列メソッド（map, filter, reduce など）を優先する
  - 省略記法を活用する
  - `for` 文の使用を避ける（必要な場合は例外）
...
```json
{
  ...
  "name": "[レッスン名]",
  "courseId": [該当するコースのID],
  "caution": "[コーディングルールや注意点]"
}
```

また、以下のフォーマットに従い、JavaScript の question データを作成してください。
初級、中級、上級でそれぞれ 50 問ずつ作ってください。

【条件】

- JavaScript の学習レベル: **{初級 | 中級 | 上級}**
- 問題のテーマ: **{関数の作成 | 配列操作 | 文字列操作 | オブジェクト操作 | 高階関数 | 非同期処理 | アルゴリズム}**
- コードの書き方: **ES6+、アロー関数を使用、配列メソッドを活用**
- できるだけ省略記法を用いて簡潔にする
- `for`文は極力使わず、適切な配列メソッド（map, filter, reduce など）を活用する
...
```json
{
  ...
  "lessonId": [レッスンのID],
  "content": "[問題文（日本語）]",
  "template": "[コードのテンプレート（未完成の状態）]",
  "title": "[問題のタイトル]",
  "example": "[具体的な入出力例]",
  "exampleAnswer": "[模範解答（JavaScriptのコード）]"
}
```
